...
weights. Each update rule hokas the same interface:
...
def rmsprop(w, dw, config=None):
    ...
    lr,dr,ep,ca=(config.get(key) for key in keys)

    config['cache']=dr*ca+(1-dr)*dw**2
...
def adam(w, dw, config=None):
    ...
    lr,bt1,bt2,ep,m,v,t=(config.get(key) for key in keys)
